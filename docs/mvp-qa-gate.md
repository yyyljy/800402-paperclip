...
- the service-check script still skips a dedicated `smoke` or `smoke:ci`
  command because no explicit smoke alias is defined yet

That means the current release gate can rely on install, type safety, test, and
build verification today, while the staged release checkpoint still depends on
platform wiring rather than on inventing new local-only checks.
...
2. `Service Checks` must continue to run frozen install, workspace typecheck,
   workspace test, and workspace build from the repository root.
...
The current release-signoff checkpoint still depends on two upstream inputs.
...
- [CMP-131](/CMP/issues/CMP-131) is still blocked, so QA does not yet have the
  full staged release-validation path
- [CMP-135](/CMP/issues/CMP-135) still needs to yield the first staging base
  URL, immutable artifact promotion path, and any environment-specific smoke
  credentials
- until those inputs exist, `MVP Slice 1 QA Signoff` is defined and locally
  verifiable, but it cannot be executed end-to-end against staging
...
1. finish [CMP-131](/CMP/issues/CMP-131) so the staged artifact path and smoke
   validation flow become real release gates
2. finish [CMP-135](/CMP/issues/CMP-135) so QA receives the first staging base
   URL, immutable artifact promotion path, and smoke-only credentials
3. expand the regression matrix when step 2, step 3, `apps/api`,
