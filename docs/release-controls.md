...
- That same `Service Checks` job now also executes the explicit root
  `smoke:ci` entrypoint, so the blocking web smoke suite runs in the protected
  release path instead of behind a separate optional workflow.
...
- The current repository wiring resolves that detection at root `smoke:ci`,
  which delegates to the `apps/web` smoke suite used for MVP release coverage.
